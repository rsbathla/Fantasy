#!/usr/bin/env python3
"""
build_flags_TE.py -- player-by-player, flag-based TE ceiling model.

Produces boom/flags_TE.json per the spec in boom/AGENT_BRIEF.md.
TE booms are the most matchup/TD-driven of any position:
  SWING soft/tough = 4.1x (vs QB 3.2, RB 1.8, WR 2.2)
So soft-TE-tier and red-zone role carry outsized weight here.

TE ARCHETYPES (each gets a truly different flag set):
  VOLUME_TE1    -- tgt_share >=0.22; #1 read; ceiling via volume + coverage-proof combos
  MIDVOL_BOOM   -- tgt_share 0.16-0.22 + boom_pctl >=70; volume AND TD upside
  COV_PROOF     -- cov_proof_pctl >=80; wins man mismatches reliably, not just volume
  YAC_THREAT    -- yac_pctl >=75 + YACoe > 0; after-catch playmaker
  TD_DEPENDENT  -- boom_pctl >=70, tgt_share <0.16; pure spike, soft-TE-tier everything
  SEAM          -- aDOT or YPRR-driven; route efficiency + air-yard ceiling
  MIDVOL        -- moderate share, moderate stats; balanced ceiling
  LOW_VOL       -- tgt_share <0.13; TD lightning only
"""
import sys, json, os
from boom_lib import load, players, reg_base, prob, label, write, cap, SWING

sm, gl, sch, de, bd = load()
posbase = bd['posbase']   # TE posbase ~= 0.182
POS = 'TE'
SPIKE = bd['SPIKE'][POS]  # 16.3 -- TE ceiling threshold
...
# Defense coverage shell: real single/two-high rates per team (FP QB Coverage Matchup, 2024 charting)
# SHELL[team] = {man, c2, c3, c4, c6, single_high, two_high, single_high_pctl (0-100)}
...
def build_line(name, arch, tgt_share, boom_pctl, cov_proof, yac_pctl, YACoe,
               YPRR, airpct, ceiling_pctl, oline_pctl, adv_pctl, spike_pctl):
...
def main():
    ...
    for key in keys:
        ...
        if base is None:
            fus = v.get('fus') or {}
            ceiling_pctl = fus.get('ceiling_pctl', 50)
            base = cap(posbase['TE'] * (0.75 + 0.5 * ceiling_pctl / 100), 0.06, 0.45)
            base = round(base, 3)
            hist = False

        # --- skill flags ---
        skill_flags, of_ids, line = build_skill_flags(key, v)
        of_total = len(of_ids)

        # --- empirical summary ---
        emp = empirical_str(key)

        # --- 18-week schedule ---
        schedule = sch.get(team, [])
        if not schedule:
            schedule = [{'wk': w, 'opp': 'BYE', 'home': None, 'dome': None} for w in range(1, 19)]
        wk_map = {g['wk']: g for g in schedule}
        weeks_out = []
        for wk in range(1, 19):
            if wk not in wk_map:
                weeks_out.append({'wk': wk, 'opp': 'BYE', 'home': None, 'dome': None,
                                   'p': None, 'lab': 'BYE', 'lit': 0, 'of': of_total, 'flags': []})
                continue
            game = wk_map[wk]
            opp  = game.get('opp', 'BYE')
            if opp == 'BYE' or opp is None:
                weeks_out.append({'wk': wk, 'opp': 'BYE', 'home': None, 'dome': None,
                                   'p': None, 'lab': 'BYE', 'lit': 0, 'of': of_total, 'flags': []})
                continue

            def_data = de.get(opp)
            mults, lit_flags = week_mults(v, def_data, game, skill_flags, of_ids, base, opp=opp)
