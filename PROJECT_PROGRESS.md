...
- 2026-07-01: Pull #2 (`build_pull_20260701.py` → `x_pull_20260701.json`, 31 tweets) merged → store **48→78** (spans 06-30 11:00 → 07-01 01:33); narrative 16→22 takes, media 4→7 items. **"Full week" investigation — DEFINITIVE:** browser cannot backfill a week. List timeline = fixed ~14h buffer; Following feed diluted + follow rate-limit; v1.1 search/timeline APIs return empty; **GraphQL SearchTimeline replay is 404 even replaying X's exact captured request verbatim** — X requires a per-request `x-client-transaction-id` from obfuscated JS that can't be replicated; search UI paginates the week but only via scroll (doesn't scale to 48 analysts). A true 7-day pull needs the paid API (user's machine) or the local scroller. User chose **accumulate forward**: store persisted to the device (`Downloads/ff_dossier_x_store.json`) so it survives container reclamation; on-demand pulls grow it into a rolling week (auto-scheduling not viable — no git remote + browser needs the desktop's Chrome open).
